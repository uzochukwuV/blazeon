...
export const withdraw = async ({
  ...
  // Get current block height to check timelock
  const currentBlock = await provider.getBlockHeight();
  if (BigInt(currentBlock) < params.unlockBlock) {
    throw new Error(`Timelock not expired. Current block: ${currentBlock}, unlock block: ${params.unlockBlock}`);
  }

  const builder = new TransactionBuilder({ provider })
    .addInputs(contractUtxos, contract.unlock.withdraw(placeholderPublicKey(), placeholderSignature()))
  ...
  const result = await WrapBuilder(builder, connector).send({
    userPrompt: `Sign to withdraw from TimeLockVault`,
    broadcast: false,
  });
